...
bool track_add_point(track *t, trackpoint *to_add)
{
  ...
  if (t->tail.prev->data == NULL) // if there is not a track besides the head and tail
  {
    latest_time = -100000; // then you can add any track
  }
  else
  {
  ...
  if (added != NULL)
  {
    ...
    else if (latest_time < trackpoint_get_time(to_add)) // if the new track comes after the latest time
    {
  ...
  {
    returnval = 3;
    // printf("added is bad: returning false!\n");
  }
  return false;
}
