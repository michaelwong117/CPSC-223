...
void track_merge(track *dest, track *src)
{
  ...
  while (srccurr != &src->tail)
  {
    ...
    // this is to handle segfault case in which src or dest has a messed up structure
    // if (srccurr == NULL)
    // {
    //   src->tail.prev = srccurr->prev;
    //   srccurr->prev->next = src->tail.prev;
    //   srccurr = &src->tail;
    //   break;
    // }
    // if (destcurr == NULL)
    // {
    //   dest->tail.prev = destcurr->prev;
    //   destcurr->prev->next = dest->tail.prev;
    //   destcurr = &dest->tail;
    //   break;
    // }

    // this should trigger if destcurr == &dest->tail
...
double track_closest_approach(const track *track1, const track *track2)
{
  ...
  while (trackpoint_compare(curr1->data, track2->head.next->data) == -1) { curr1 = curr1->next; }
  
  while (trackpoint_compare(curr2->data, track1->head.next->data) == -1) { curr2 = curr2->next; }

  while (trackpoint_compare(t1end->data, track2->tail.prev->data) == 1) { t1end = t1end->prev; }

  while (trackpoint_compare(t2end->data, track1->tail.prev->data) == 1) { t2end = t2end->prev; }
